...
distFunc :: Int -> [[Cell]] -> ((Int, Int), (Int, Int)) -> Int
distFunc scale grid = f
  where
    emptyRows = findEmptyRows grid
    dpRows = scanl (+) 0 $ map (\y -> bool 1 scale (y `elem` emptyRows)) [0 ..]
    emptyCols = findEmptyRows (transpose grid)
    dpCols = scanl (+) 0 $ map (\x -> bool 1 scale (x `elem` emptyCols)) [0 ..]
    oneD checkList (a, b) =
