...
answerFunc all_data =
  let lined = lines all_data
   ...
   in length . filter ((== Sand) . snd) . M.toList $ a


simulateStep :: State -> (Bool, Bool, Pos) -- (stopped, bounded, state)
simulateStep state@(grid, sandPos) = (isNothing newPos && sandPos == (500, 0), isNothing newPos, fromMaybe (500, 0) newPos)
  where
...
createInitState lst = (M.insert (500, 0) Sand (foldl helper initGrid (floor:lst)), (500, 0))
  where
    b@((miX, miY), (maX, maY)) = calcBounds lst
    floor = ((0, maY + 2), (maX*3, maY + 2))
    initGrid = foldl (\grid coord -> M.insert coord Empty grid) M.empty [(x, y) | x <- [0 .. maX*3], y <- [miY .. maY+3]]
